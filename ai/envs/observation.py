...
class ObservationEncoder:
    ...
    每个 region 编码为 F 维：
      owner_onehot (max_players+1) | troops_norm | is_capital | base_growth_norm | is_visible

    owner 使用 viewer-relative one-hot：
      index 0 = neutral, index 1 = viewer 自己, index 2+ = 其他玩家（player_id 升序）

    use_adjacency=True 时追加整张地图的邻接矩阵（N*N 维，按行展开），N 为地区数。
    全邻接矩阵放在 region 特征之后、全局特征之前。
    """
    ...
    def __init__(self, game_map: GameMap, max_players: int,
        ...
        if use_adjacency:
            self.dim += self._num_regions * self._num_regions
            self._adjacency_matrix = game_map.adjacency_matrix.flatten()
    ...
    def encode(
        ...
        if self._use_adjacency:
            adj_start = self._num_regions * F
            adj_end = adj_start + self._num_regions * self._num_regions
            vec[adj_start:adj_end] = self._adjacency_matrix
