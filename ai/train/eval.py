...
用法：
    ...
    results = evaluate(ckpt, venv, episodes_per_env=20, opponent_specs=specs)
...
def evaluate(
    ...
    调用方负责在评估前设置每个 env 的对手和首都。

    Args:
        agent_path: checkpoint 路径（不含 .zip 后缀）
        venv: 已配置对手 + 首都的 VecEnv（通常就是训练用的 env）
        episodes_per_env: 每个 env 跑的局数
        opponent_specs: 每 env 一 spec，用于 EvalResult 元数据
